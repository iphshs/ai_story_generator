...
LOG_DIR = "./log"
STORIES_DIR = "./stories"
N_TURNS = 10 # Number of turns in the agent debate (can be <= number of agents defined)
...
def get_user_input(use_defaults=False):
    ...
    if use_defaults:
        ...
        # Ensure nested dictionaries/lists are copied correctly if needed, though here it's simple init
...
class Agent:
    ...
    def _simulate_llm_call(self, prompt, current_state):
        ...
        if "Storyteller" in self.name:
            ...
            detail = f"Detail: The {current_state.get('setting', 'setting')} has a unique feature like {current_state.get('genre', 'a genre-specific element')}." # Made slightly more descriptive
            ...
            character_list = str(current_state.get('characters', 'Char1')).split(',')
            char1 = character_list[0].strip()
            dialogue = f"Dialogue Snippet: '{char1}' said something simple and engaging, fitting age {current_state.get('age_appropriate', 'range')}." # Made slightly more descriptive
            ...
            logging.info("Final Preprint Editor: Assembling the final story...") # Log entry
            story_parts = []
            ...
            # Weave in elements - improved basic weaving for simulation
            ...
            # Add scenes, interleaving dialogue and prompts
            scenes = elements.get('scenes', ['Once upon a time...'])
            dialogues = elements.get('dialogue', [])
            dialogue_idx = 0
            for i, scene in enumerate(scenes):
                story_parts.append(scene)
                # Add a prompt after the scene (adjust logic as needed)
                if prompt_idx < len(prompts):
                    # Simple distribution: add prompt after first scene, then maybe later
                    if i == 0 or (i > 0 and i % 2 == 0) : # Example: prompt after scene 1, 3, 5...
                       story_parts.append(f"\n{prompts[prompt_idx]}\n")
                       prompt_idx += 1
                # Add dialogue snippets after scenes where prompts weren't added
                elif dialogue_idx < len(dialogues):
                    story_parts.append(dialogues[dialogue_idx])
                    dialogue_idx +=1

            # Add any remaining dialogue/prompts
            while dialogue_idx < len(dialogues):
                story_parts.append(dialogues[dialogue_idx])
                dialogue_idx +=1
            ...
            final_story_text = "\n\n".join(story_parts)
            # Ensure final story is not empty before assigning
            if final_story_text.strip():
                current_state['final_story'] = final_story_text
                logging.info("Final Preprint Editor: Successfully assembled final story.") # Log success
                return "Compiled final story from elements, added prompts, and performed final review."
            else:
                logging.warning("Final Preprint Editor: Assembled story appears empty. Skipping update.")
                return "Attempted to compile final story, but result was empty."
            # --- End Simulation ---
        else:
            return f"Agent {self.name} contributing..."

# --- Specific Agent Implementations ---
# (No changes needed in the specific agent classes themselves for this fix)
...
def run_agent_pipeline(user_preferences):
    ...
    # The Final Preprint Editor should always be the last one in this list.
    agents = [
        ...
        StorytellerAgent("Storyteller - Ending"), # Added ending storyteller
        # Add more turns for refinement if needed, keeping editor last
        FinalPreprintEditorAgent("Final Preprint Editor") # Ensure this is always last
    ]
    ...
    # Determine the number of turns based on N_TURNS and available steps
    # Ensure at least one turn runs if N_TURNS > 0
    actual_turns = min(N_TURNS, num_pipeline_steps) if N_TURNS > 0 else 0

    logging.info(f"Running pipeline for {actual_turns} turns out of {num_pipeline_steps} defined steps.")

    if actual_turns <= 0:
        logging.warning("N_TURNS is 0 or less, skipping agent pipeline.")
        return current_state # Return initial state if no turns

    for i in range(actual_turns):
        # Determine the agent for the current turn
        # *** CORRECTED LOGIC ***
        if i == actual_turns - 1:
            # On the very last turn of the loop, always use the last agent defined in the list
            agent = agents[num_pipeline_steps - 1]
            agent_log_name = f"{agent.name} (Final Turn)"
        else:
            # For all other turns, use the agent corresponding to the turn index 'i'
            # This ensures we don't skip agents if N_TURNS < num_pipeline_steps
            agent = agents[i]
            agent_log_name = agent.name
        # *** END CORRECTED LOGIC ***

        logging.info(f"\n--- Turn {i+1}/{actual_turns} | Agent: {agent_log_name} ---")
        ...
        try:
            ...
            # Ensure response_description is a string before creating utterance
            if not isinstance(response_description, str):
                 response_description = str(response_description) # Convert if needed
    ...
    # Final check if story exists, create fallback if editor failed silently or result was empty
    if 'final_story' not in current_state or not current_state.get('final_story', '').strip():
         ...
         elements = current_state.get('story_elements', {})
         prompts = current_state.get('image_prompts', [])
         fallback_parts.extend(elements.get('setting_details', []))
         fallback_parts.extend(elements.get('character_notes', []))
